...
public class PMudSpellsHandler extends HandlerBase {
    ...
    public void handleWhereSpell(VerbRequest req, VerbResponse response) {
        ...
        if (f != null) {
            response.setToSender(String.format("[%5d]%25s - %-30s| %s", f.getId(), f.getName(), f.getLocation().getTitle(), f.getLocation().getName2()));
            return;
        }
        ...
        if (f2.size() > 0) {
            String msg = "";
            for (PObject o : f2) {
                msg += String.format("[%5d]%25s - %-30s| %s\n", o.getId(), o.getpName(), o.getLocation().getTitle(), o.getLocation().getName2());
            }
